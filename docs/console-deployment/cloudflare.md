...
Complete [Basics](README.md) first. This guide uses native D1/R2 bindings.

## 1. Copy the Worker example
...
The default origin is `https://<worker-name>.<account-subdomain>.workers.dev`. Register its [OAuth callback](README.md#3-set-up-sign-in), or use your custom domain consistently.
...
Fill the values from [sign-in setup](README.md#3-set-up-sign-in). Generate a separate storage secret with `openssl rand -base64 32`.
...
Wrangler reports the deployed URL. Open it and follow [Verify the deployment](README.md#5-verify-the-deployment).
...
<details>
<summary>Update or roll back</summary>
