...
Updater provider environment variables. The default compatibility flag is
`nodejs_als`, which is enough for TanStack Start's async request context. If a
provider package in your `hot-updater.config.ts` still requires broader Node.js
APIs, use `nodejs_compat` as a fallback compatibility flag for that deployment.

## Access Control

The console includes a Better Auth email/password gate and protects every
console API server function with the active session. You can still place the
